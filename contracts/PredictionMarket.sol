...
contract PredictionMarket is Ownable {
    ...
    struct ConditionInfo
    {
        ...
        address[] totalAboveParticipants;
        address[] totalCallParticipants;
        bool isSettled;
        int settledPrice;
    }
    ...
    event ConditionPrepared(
        address indexed conditionIndex,
    ...
    function prepareCondition(address _oracle,uint256 _settlementTime, int _triggerPrice) external onlyOwner{
        ...
        require(conditionInfo.oracle !=address(0), "condition already prepared");
        ...
        emit ConditionPrepared(conditionIndex, _oracle, _settlementTime, triggerPrice);
    }
    ...
    function betOnCondition(uint256 _conditionIndex,uint8 _prediction) public payable{
        ...
        uint256 userAddress = msg.sender;
        ...
        userInfo.ETHStaked = userInfo.ETHStaked.add(userETHStaked);
        if(_prediction == 0) {
            conditionInfo.putParticipants.push(userAddress);
            conditionInfo.putETHStaked = conditionInfo.putETHStaked.add(userETHStaked);
        }
        else{
            conditionInfo.callParticipants.push(userAddress);
            conditionInfo.callETHStaked = conditionInfo.callETHStaked.add(userETHStaked);
        }
        userInfo.bet = _prediction;
    ...
    function claim(uint256 _conditionIndex) public{
        ...
        uint256 winnerTotalETHStaked;   //total eth staked by the winning side
        uint256 userStake;
        if(conditionInfo.settledPrice > conditionInfo.triggerPrice){    //Users who predicted above price wins 
            totalPayout = conditionInfo.belowETHStaked;
            winnerTotalETHStaked = conditionInfo.aboveETHStaked;
            
                winnerPayout = winningETHPool.mul(userInfo.ETHStaked.div(winnerTotalETHStaked));
                (conditionInfo.callParticipants[i]).transfer()
            }
        }
        if(conditionInfo.settledPrice < conditionInfo.triggerPrice){      //Users who predicted below price wins
            winningETHPool = conditionInfo.;
